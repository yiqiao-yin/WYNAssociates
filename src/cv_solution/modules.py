...
class YinsCV:
    ...
    # define unet
    def shallow_unet_model(input_shape=(128, 128, 3)):
        # define conv_block
    ...
    def multi_unet_model(
        n_classes=4, IMG_HEIGHT=256, IMG_WIDTH=256, IMG_CHANNELS=1, useSCALE=False
    ):
        # Build the model
        inputs = Input((IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
        if useSCALE:
            s = Lambda(lambda x: x / 255)(
                inputs
    ...
    def inception_unet_model(
        img_size=(128, 128), num_classes=2, ENC_PARAM=[64, 128, 256]
    ):
    ...
    def unet_inceptionstyle_model(
        ...
        if plotModel:
            ...
            tf.keras.utils.plot_model(
                model, to_file=name_of_file, show_shapes=True, expand_nested=True
            )
        ...
        # callbacks
        callbacks = [
            keras.callbacks.ModelCheckpoint(name_of_model + ".h5", save_best_only=True)
        ]
        # note
        ...
        datagen.fit(
            x_train
        )  # <= this should only be training data otherwise it is cheating!
        ...
        if verbose:
            ...
            print(
                "Please choose a positive interger up to " + str(len(model.layers) - 1)
            )
        if which_layer != None:
            from tensorflow.keras import backend as K

            get_internal_layer_fct = K.function(
                [model.layers[0].input], [model.layers[which_layer].output]
            )
        ...
        return {
            ...
            "Prediction": {"y_hat_train_": y_hat_train_, "y_hat_train_": y_hat_train_},
        }
    ...
    def superimposedImages(
        ...
        superimposed_img_pil = tf.keras.preprocessing.image.array_to_img(
            superimposed_img
        )
